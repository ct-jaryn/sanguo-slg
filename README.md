...
- 全面玩法/剧情审查与修复（详见 `docs/audit-and-fixes.md`，共 48 项）：事件系统玩家选择恢复（原先生成当回合即被自动结算）、围城战术崩溃、断粮白嫖维持费、幽灵武将/幽灵军团、坏存档污染对局、守方技能与羁绊空转、攻城凭空造兵、AI 自残与战果错算、经济数值失控、历史事件时序/史实修正等。
...
└── js/                     # JavaScript 模块
    ...
    └── ui/                 # UI 渲染
        ├── renderer.js     # Tab 分发与内容渲染总控
        ├── common.js       # 通用渲染、弹窗、特效、提示
        └── tabs/
```
...
| `js/core/` | 全局状态管理、通用工具函数、战斗计算、日志 |
| `js/systems/` | AI、经济、事件、成就、教程、存档、音效 |
| `js/ui/` | 渲染总控、通用 UI、弹窗、各功能面板 |
